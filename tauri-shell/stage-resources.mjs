...
// 排除 .map/.ts）。手维护清单曾漏装 lib/logger、lib/plugin-guard、
// lib/client-update、lib/renderer-recovery（根 logger.js/plugin-guard.js 是
...
console.log('[stage] vnext 隔离体系（整个 lib/ 运行产物递归装配 + shared 协议 + 原生 .node）');
copyLibTree();
...
// 整目录拷贝（Windows: vendor/node/node.exe；Linux/macOS: vendor/node/bin/node，
// 由 fetch-node 按平台生成）；Linux 补充可执行位（官方 tarball 已带，双保险）。
