...
- **独立悬浮窗**（无侧栏停靠形态）：`position:fixed` 可拖拽移动、右下角缩放手柄；
  始终约束在当前视口内；启动不自动弹出，由左下角 💬 图标 /
  `Ctrl+Shift+S` / `/side-session` 斜杠命令唤起。
- **左边栏图标与设置按钮同款**：宽态整行（图标+文字）、收起态圆形图标按钮，
  同 hover/按压交互，无焦点黑框；打开浮窗时高亮；与知识库等多个 footer
  入口同时启用时纵向排列，不会被推到侧栏外。
...
- **流式回答**：浮窗隐藏时暂停轮询（省资源）。
- **三种回答引擎**（互斥、持久化、即时切换）：
  1. 直连 DeepSeek 官方——读 `DEEPSEEK_API_KEY` 环境变量 / `$DSH_HOME/.credentials.yaml`
  2. 插件自带 Key（apiKey / model / endpoint，secret 持久化）
  3. 跟随主对话模型（默认、推荐）——优先读取当前会话的待用/最近模型，通过
     `ctx.llm` 使用同一 provider/model，不单独读取 key
...
- **0.2.8**（2026-09-03）：修复知识库等 footer action 占满整行后把临时对话
  导航入口横向挤出侧栏的问题；多个入口统一纵向排列，宽态整行、收起态居中。
- **0.2.7**（2026-09-03）：修复第二轮临时对话把 assistant 历史直接交给宿主 LLM
  导致的 `kind` 读取错误；历史折叠为纯文本上下文，仅发送最后一条用户消息；
  浮窗在打开、拖动、缩放和视口变化后都会完整约束在可见区域。
- **0.2.6**（2026-09-02）：默认回答引擎恢复为宿主 LLM；优先读取当前会话的
  `modelSelection.pending/lastUsed`，保证临时对话跟随主界面模型；直连 DeepSeek
  模式不再转发第三方模型 id；上游错误保留真实 HTTP 状态和错误正文。
