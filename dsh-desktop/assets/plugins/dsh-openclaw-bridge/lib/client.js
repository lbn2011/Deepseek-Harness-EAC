...
window.__ModuleLoader__.load({
	...
	//  http://www.denso-wave.com/qrcodeGen/faqpatent-e.html
	//
	//---------------------------------------------------------------------
	...
	var qrcodeGen = function() {
	   ...
	   */
	  var qrcodeGen = function(typeNumber, errorCorrectionLevel) {
	    ...
	    _this.createSvgTag = function(cellSize, margin, alt, title) {
	      ...
	      alt.id = (alt.text) ? alt.id || 'qrcodeGen-description' : null;
	
	      // Compose title property surrogate
	      title = (typeof title === 'string') ? {text: title} : title || {};
	      title.text = title.text || null;
	      title.id = (title.text) ? title.id || 'qrcodeGen-title' : null;
	  ...
	  //---------------------------------------------------------------------
	  // qrcodeGen.stringToBytes
	  //---------------------------------------------------------------------
	
	  qrcodeGen.stringToBytesFuncs = {
	    'default' : function(s) {
	      var bytes = [];
	      for (var i = 0; i < s.length; i += 1) {
	  ...
	  qrcodeGen.stringToBytes = qrcodeGen.stringToBytesFuncs['default'];
	
	  //---------------------------------------------------------------------
	  // qrcodeGen.createStringToBytes
	  //---------------------------------------------------------------------
	   ...
	   * @param numChars
	   */
	  qrcodeGen.createStringToBytes = function(unicodeData, numChars) {
	  ...
	  var QRUtil = function() {
	    ...
	    _this.getLostPoint = function(qrcodeGen) {
	
	      var moduleCount = qrcodeGen.getModuleCount();
	      ...
	      for (var row = 0; row < moduleCount; row += 1) {
	        for (var col = 0; col < moduleCount; col += 1) {
	          ...
	          var sameCount = 0;
	          var dark = qrcodeGen.isDark(row, col);
	          ...
	          for (var r = -1; r <= 1; r += 1) {
	            ...
	            for (var c = -1; c <= 1; c += 1) {
	              ...
	              if (dark == qrcodeGen.isDark(row + r, col + c) ) {
	                sameCount += 1;
	              }
	            }
	          }
	      ...
	      for (var row = 0; row < moduleCount - 1; row += 1) {
	        for (var col = 0; col < moduleCount - 1; col += 1) {
	          var count = 0;
	          if (qrcodeGen.isDark(row, col) ) count += 1;
	          if (qrcodeGen.isDark(row + 1, col) ) count += 1;
	          if (qrcodeGen.isDark(row, col + 1) ) count += 1;
	          if (qrcodeGen.isDark(row + 1, col + 1) ) count += 1;
	          if (count == 0 || count == 4) {
	            lostPoint += 3;
	          }
	        }
	      ...
	      for (var row = 0; row < moduleCount; row += 1) {
	        for (var col = 0; col < moduleCount - 6; col += 1) {
	          if (qrcodeGen.isDark(row, col)
	              && !qrcodeGen.isDark(row, col + 1)
	              &&  qrcodeGen.isDark(row, col + 2)
	              &&  qrcodeGen.isDark(row, col + 3)
	              &&  qrcodeGen.isDark(row, col + 4)
	              && !qrcodeGen.isDark(row, col + 5)
	              &&  qrcodeGen.isDark(row, col + 6) ) {
	            lostPoint += 40;
	          }
	        }
	      }
	      ...
	      for (var col = 0; col < moduleCount; col += 1) {
	        for (var row = 0; row < moduleCount - 6; row += 1) {
	          if (qrcodeGen.isDark(row, col)
	              && !qrcodeGen.isDark(row + 1, col)
	              &&  qrcodeGen.isDark(row + 2, col)
	              &&  qrcodeGen.isDark(row + 3, col)
	              &&  qrcodeGen.isDark(row + 4, col)
	              && !qrcodeGen.isDark(row + 5, col)
	              &&  qrcodeGen.isDark(row + 6, col) ) {
	            lostPoint += 40;
	          }
	        }
	      }
	  ...
	  var qr8BitByte = function(data) {
	    ...
	    var _data = data;
	    var _bytes = qrcodeGen.stringToBytes(data);
	  ...
	  var qrKanji = function(data) {
	    ...
	    var stringToBytes = qrcodeGen.stringToBytesFuncs['SJIS'];
	    if (!stringToBytes) {
	...
	!function() {
	  ...
	  qrcodeGen.stringToBytesFuncs['UTF-8'] = function(s) {
		...
		// 本地二维码渲染：5.3.2 及以前把微信绑定二维码（等价登录凭据）拼给
		// 第三方 api.qrserver.com 渲染 —— 凭据外泄 + 断网/被墙即白屏。现用
		// 随包内置的 qrcode-generator（MIT，与 dsh-phone 同源）本地生成 dataURL。
		...
		function SettingsBlock(props) {
			...
			const save = async () => {
				...
				try {
					...
					// 失败必须上屏：React 事件里的 async 拒绝是 unhandled
					// rejection，用户点保存会毫无反馈。
				...
				children: [
					...
					jsxs("div", {
						...
						children: [
							...
							saveErr ? jsx("span", { style: { color: "var(--dsw-alias-state-error-primary, #e5484d)", fontSize: 12 }, children: saveErr }) : null
						]
					})
				]
			});
		...
		function WechatBlock() {
			...
			// 本地生成（见上方 qrDataUrl）：不再把绑定二维码发给第三方渲染。
			const qrImg = st && st.qrcodeUrl ? qrDataUrl(st.qrcodeUrl) : null;
